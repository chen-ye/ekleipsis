import { EyeOpenIcon, HomeIcon, SewingPinIcon } from '@radix-ui/react-icons';
...
export default function PoiLayer({
	...
	onPoiClick?: (position: Cartesian3) => void;
	clampTo3DTiles?: boolean;
}) {
	const { pois } = usePoi();
		...
		<>
			{pois.map((poi) => {
				if (poi.type === 'track' && poi.path) {
					const positions = poi.path.map((p) =>
				...
				return (
					<Entity
						...
						billboard={{
							...
							heightReference: clampTo3DTiles
