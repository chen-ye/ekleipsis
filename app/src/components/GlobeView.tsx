...
function GlobeView({ cameraDestination, onFlyTo }: GlobeViewProps) {
	...
	const { startJD, endJD, totalityStartJD, totalityEndJD } = useMemo(() => {
		...
		return {
			...
			totalityStartJD: eclipseTiming.totalityStartTime
				? JulianDate.fromDate(eclipseTiming.totalityStartTime)
				: undefined,
			totalityEndJD: eclipseTiming.totalityEndTime
				? JulianDate.fromDate(eclipseTiming.totalityEndTime)
				: undefined,
		};
	}, [eclipseTiming]);
	...
	const coverageData = useMemo(() => {
		...
		for (let i = 0; i <= steps; i++) {
			...
			const coverage = calculateEclipseCoverage(
				time,
				MALLORCA_LAT,
				MALLORCA_LNG,
			);
	...
	useEffect(() => {
		...
		const updateSkyBrightness = (clock: CesiumClock) => {
			...
			if (
				...
				JulianDate.lessThan(time, totalityStartJD)
			) {
			...
			if (
				totalityEndJD &&
				JulianDate.greaterThan(time, totalityEndJD) &&
				JulianDate.lessThanOrEquals(time, endJD)
			) {
	...
	}, [viewer, startJD, endJD, totalityStartJD, totalityEndJD]);
		...
		<div style={{ width: "100%", height: "100%" }}>
			...
			>
				...
				>
					...
					<SegmentedControl.Item value="cesium">
						Cesium World
					</SegmentedControl.Item>
				</SegmentedControl.Root>
			...
			>
				<ShadowMap
					...
					softShadows={true}
				/>
				<Scene highDynamicRange={false} logarithmicDepthBuffer={true} />
				<Globe
					enableLighting={true}
