 ...
 */
export function getEclipseTiming(
	date: Date,
	lat: number,
	lng: number,
	elevation: number = 0,
 ...
 */
export function calculateEclipseCoverage(
	date: Date,
	lat: number,
	lng: number,
	elevation: number = 0,
	...
	const sunRadiusDeg =
		(Math.asin(SUN_RADIUS_KM / (sunDistAU * AU_KM)) * 180) / Math.PI;
	const moonRadiusDeg =
		(Math.asin(MOON_RADIUS_KM / (moonDistAU * AU_KM)) * 180) / Math.PI;
